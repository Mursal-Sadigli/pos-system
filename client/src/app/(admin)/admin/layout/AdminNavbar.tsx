      ...
      <div className="mx-auto flex h-16 items-center gap-4 px-4 md:px-6">
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin" className="flex items-center gap-2">
            ...
            <span className="hidden text-lg font-semibold sm:inline">POS sistemi <span className="text-sm font-normal text-muted-foreground">Admin</span></span>
          </Link>

          <nav className="hidden text-sm text-muted-foreground lg:flex gap-2">
            {breadcrumbs.map((item, index) => (
              <span key={item.href} className="flex items-center gap-2">
                <Link href={item.href.replace('/dashboard', '/admin')} className="hover:text-foreground">
                  {item.label}
                </Link>
